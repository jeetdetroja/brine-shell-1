...
// Status. See backend/README.md for the header row to use.
...
/* Every order whose Email column matches the logged-in user's email.
   This is how orders are "linked to an account" without needing a
   separate user table — the email from checkout IS the identity. */
router.get('/mine', requireAuth, async (req, res) => {
  ...
  const orders = rows
    ...
    .filter(row => row[COL.paymentStatus] === 'paid' && (row[COL.email] || '').toLowerCase() === email)
    .map(rowToSummary)
...
router.get('/:orderId', requireAuth, async (req, res) => {
  ...
  if (!row || (row[COL.email] || '').toLowerCase() !== req.user.email.toLowerCase()) {
